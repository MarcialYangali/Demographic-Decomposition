...
El objetivo central de esta sección es comparar dos enfoques clásicos y ampliamente utilizados en la descomposición demográfica de tasas: el método algebraico-simétrico de Das Gupta y el método de perturbación continua de Horiuchi, Wilmoth y Pletcher. Ambos permiten descomponer diferencias en indicadores agregados en función de sus múltiples determinantes, pero lo hacen desde principios matemáticos distintos, lo que potencialmente produce diferencias en interpretación, estabilidad y sensibilidad.

A diferencia de Kitagawa (1955), cuya formulación sólo opera para dos factores y en estructuras del tipo media ponderada, tanto Das Gupta (1978, 1991, 1993) como Horiuchi (2008) permiten trabajar con tres o más determinantes simultáneos, condición indispensable para nuestro caso de estudio, que requiere descomponer diferencias en la exposición al calor considerando simultáneamente edad, discapacidad y tasas específicas.

En ausencia de comparaciones sistemáticas en la literatura —ningún estudio ha evaluado formalmente las diferencias entre ambos métodos bajo un mismo conjunto de datos y función de tasa— en esta sección se propone un contraste riguroso entre ambos enfoques aplicados al mismo indicador demográfico.

```{r}
# ==================================================
# Data preparation: Age × Disability × Heat rate
# ==================================================



prepare_threeway_data <- function(df) {
  
  out <- df |>
    group_by(region, age5) |>
    summarise(
      S = sum(pop * 366, na.rm = TRUE),      # person-days
      D = sum(popdis * 366, na.rm = TRUE),   # disabled person-days
      H = sum(pop * heatdays, na.rm = TRUE), # heat person-days
      .groups = "drop"
    ) |>
    group_by(region) |>
    mutate(
      s = S / sum(S, na.rm = TRUE),  # age structure
      d = D / S,                     # disability prevalence
      h = H / S                      # heat rate
    ) |>
    ungroup()
...
# ==================================================
# Three-way symmetric decomposition (sequential averages)
# ==================================================

threeway_decomposition <- function(data, ref_region, cmp_region) {
  
  ref <- data |>
    filter(region == ref_region) |>
    select(age5, s_ref = s, d_ref = d, h_ref = h)
  
  cmp <- data |>
    filter(region == cmp_region) |>
    select(age5, s_cmp = s, d_cmp = d, h_cmp = h)
  
  merged <- full_join(ref, cmp, by = "age5") |>
    mutate(across(where(is.numeric), ~replace_na(.x, 0)))
  
  # total rate
  R_ref <- sum(merged$s_ref * merged$d_ref * merged$h_ref) * 100
  R_cmp <- sum(merged$s_cmp * merged$d_cmp * merged$h_cmp) * 100
  
  # -----------------------------
  # Order 1: s → d → h
  # -----------------------------
  R1_1 <- sum(merged$s_cmp * merged$d_ref * merged$h_ref) * 100
  R1_2 <- sum(merged$s_cmp * merged$d_cmp * merged$h_ref) * 100
  ...
  # Order 2: h → d → s
  # -----------------------------
  R2_1 <- sum(merged$s_ref * merged$d_ref * merged$h_cmp) * 100
  R2_2 <- sum(merged$s_ref * merged$d_cmp * merged$h_cmp) * 100
  ...
  # Order 3: d → s → h
  # -----------------------------
  R3_1 <- sum(merged$s_ref * merged$d_cmp * merged$h_ref) * 100
  R3_2 <- sum(merged$s_cmp * merged$d_cmp * merged$h_ref) * 100
  ...
  tibble(
    ...
    R_ref = R_ref,
    R_cmp = R_cmp,
    total_diff = R_cmp - R_ref,
    effect_age = eff_s,
    effect_disability = eff_d,
    effect_heat = eff_h,
    effects_sum = eff_s + eff_d + eff_h,
    check = abs((eff_s + eff_d + eff_h) - (R_cmp - R_ref))
  )
}
...
run_threeway_decomposition <- function(df, reference = "National") {
  
  data <- prepare_threeway_data(df)
  
  if (!reference %in% unique(data$region)) {
    stop(paste0("Reference region '", reference, "' not found."))
  }
  
  regions <- setdiff(unique(data$region), reference)
  
  results <- purrr::map_dfr(
    regions,
    ~threeway_decomposition(data, reference, .x)
  )
...
source("das_gupta_threeway.R")

results <- das_gupta_threeway(data_prepared, 
                              ref_region = "National",
                              cmp_region = "Baja California")
