...
Sea $x$ el índice que recorre las categorías de estratificación de cualquier variable estructural relevante: edades, niveles educativos, grupos socioeconómicos, tipos de hogares, regiones de residencia, etc.

Denotemos por $G$ cualquier población o grupo cuya tasa global queremos comparar.

Para cada combinación $(G, x)$, definimos:

1.  Tamaño del estrato

$$\eta_x^{(G)} : \text{número total de unidades en la categoría estructural } x \text{ dentro del grupo } G.$$

2\. Estructura o peso relativo del estrato

$$s_x^{(G)} = \frac{\eta_x^{(G)}}{\sum_x \eta_x^{(G)}}.$$

3\. Tasa específica del estrato

$$r_x^{(G)} : \text{tasa, proporción o intensidad correspondiente al estrato } x.$$

La tasa global del grupo $G$ es siempre una media ponderada:

$$R^{(G)} = \sum_x s_x^{(G)} \, r_x^{(G)}.$$

Comparación entre dos poblaciones

Sea $A$ y $B$ dos grupos cualesquiera. La diferencia total en las tasas globales es:

$$\Delta R = R^{(A)} - R^{(B)}.$$

El método clásico de **Kitagawa (1955)** separa exactamente estas dos fuentes de variación mediante una descomposición aditiva y simétrica:
...
1.  cambios en las **tasas específicas**, captura cuánto de la diferencia total se debe a variación en las tasas específicas entre estratos, manteniendo fija la estructura. Es la parte “pura” del cambio en el riesgo o la intensidad del evento de interés.

2.  cambios en la **composición estructural**, mide la porción explicada por diferencias en la estructura del grupo (edad, educación, región, lo que sea). Su interpretación genérica es:\

    *“¿Cuánto cambiaría la tasa global si los grupos tuvieran la misma estructura, pero conservaran sus propias tasas específicas?”*
