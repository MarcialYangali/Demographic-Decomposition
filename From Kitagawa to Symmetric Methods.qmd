...
results_rg|> 
  ...
  dplyr::arrange(desc(abs(total_diff))) |> 
  gt() |> 
  tab_header(
  ...
  fmt_number(
    columns = c(effect_heat, effect_age, effect_disability, total_diff),
    decimals = 4
  ) |> 
  data_color(
...
datos_decomp <- prepare_horiuchi_data(df, discapacidades, age5_levels)
...
exposure_table |> 
  ...
  data_color(
    ...
    colors = scales::col_numeric(
      palette = c("#2a9d8f", "white", "#d62828"),
      domain = range(exposure_table$exposure_total)
    )
  )
```
